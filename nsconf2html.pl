...
print "Service List:\n";
print $out "<table border=1pt><tr><td>Service Name</td><td>Server</td><td>Port</td>";
if($has_td==1){
print $out "<td>TD</td>";
}
print $out "</tr>";
...
while( my $line = <$info>)  {   
    ...
    if($line =~ /add lb vserver/){
        ...
        print $out "<tr><td>vs_borrar</td><td>IP</td><td>".$values[5]."</td></tr>\n";
        print $out "<tr><td>vs_borrar</td><td>Puerto</td><td>".$values[6]."</td></tr>\n";
    	############## Add aditional lines if you need more rows with information, 
    	############## params is a hash that uses the key as the -param i.e. -persistenceType without the '-' 
    	print $out "<tr><<td>vs_borrar</td><td>persistenceType</td><td>".$params{"persistenceType"}."</td></tr>\n";
    	if(exists $params{"timeout"})
    	{ 
    	 	print $out "<tr><td>vs_borrar</td><td>Persistence Timeout</td><td>".$params{"timeout"}."</td></tr>\n";
    	 }else{
    	 	print $out "<tr><td>vs_borrar</td><td>Persistence Timeout</td><td>Default</td></tr>\n";
    	 }
    	if(exists $params{"lbmethod"})
    	{ 
    		print $out "<tr><td>vs_borrar</td><td>Loadbalance Method</td><td>".$params{"lbmethod"}."</td></tr>\n";
    	}else{
    		print $out "<tr><td>vs_borrar</td><td>Loadbalance Method</td><td>ROUNDROBIN</td></tr>\n";
    	}
    	if(exists $params{"td"})
    	{   		
    		print $out "<tr><td>vs_borrar</td><td>Traffic Domain</td><td>".$params{"td"}."</td></tr>\n";
    	}
    	...
    	for my $i (0 .. @services-1){
    		#print "iteracion: ".$i." VS: ".$values[3]." Services: ".@services[$i]."\n";
    		if ($i==0){ 
    			print $out "<tr><<td>vs_borrar</td><td>Services</td><td>".$services[$i]."</td></tr>\n";
    		}else{
    			print $out "<tr><td>vs_borrar</td><td>vs_borrar</td><td>".$services[$i]."</td></tr>\n";	
    		}
    	}
    	#print "Dump hash".Dumper(\%params)."\n";
    #persistenceType
